...
If you maintain a Google Scholar profile, run `bin/update_scholar.py` to download the latest entries, refresh citation snapshots, and regenerate [`_bibliography/papers.bib`](_bibliography/papers.bib) plus [`_data/scholar.yml`](_data/scholar.yml). The script reads your `scholar_userid` from [`_data/socials.yml`](_data/socials.yml) and can also generate local publication preview images from linked PDFs.

To refresh the Scholar snapshot:

1. Locally: run `python bin/update_scholar.py` from the repository root.
2. On GitHub: open `Actions` -> `Update Scholar Data` -> `Run workflow`.
